...
print_yellow "* installing 'pcre v${VERSION}' ···"

# don't edit those variables
DOWNLOAD_URL="https://ftp.pcre.org/pub/pcre/pcre-${VERSION}.tar.gz"
DOWNLOAD_FILE="/tmp/pcre-${VERSION}.tar.gz"
SOURCE_DIR="/usr/local/src/pcre-${VERSION}"
...
if [ -d "${SOURCE_DIR}" ]; then
	...
	./configure --prefix=/usr/local --enable-utf --enable-jit
