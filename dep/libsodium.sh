...
print_yellow "* installing 'libsodium v${VERSION}' ···"

# don't edit those variables
DOWNLOAD_URL="https://download.libsodium.org/libsodium/releases/libsodium-${VERSION}-stable.tar.gz"
DOWNLOAD_FILE="/tmp/libsodium-${VERSION}.tar.gz"
SOURCE_DIR="/usr/local/src/libsodium-stable"
...
if [ -d "${SOURCE_DIR}" ]; then
	...
	make && make check && make install && cd ${LAUNCH_DIR}
